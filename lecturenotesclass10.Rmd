...
Red is a little above, Blue is a little below, and White is pretty much spot on.
...
roll_results <- roll_results %>% 
  mutate(square = die_1 ^ 2 == die_2,
         absolute = abs(die_1 - die_2) == 4)
roll_results
```

```{r summarize_rolls}
roll_results %>% 
  summarize(square_prob = mean(square),
            absolute_prob = mean(absolute))
```
